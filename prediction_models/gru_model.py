...
class CustomModel(tf.keras.Model):
    def __init__(self, num_layers, units, dropout_rate, output_dim):
        super(CustomModel, self).__init__()
        self.gru_layers = [tf.keras.layers.GRU(units, return_sequences=(i < num_layers - 1)) for i in range(num_layers)]
...
def train_model(X_train, y_train, num_layers=1, units=128, dropout_rate=0.8, output_dim=1, learning_rate=0.01,
    ...
    modelnn = CustomModel(num_layers, units, dropout_rate, output_dim)
    ...
    def train_step(batch_x, batch_y):
        ...
        optimizer.apply_gradients(zip(gradients, modelnn.trainable_variables))
        return logits, loss
