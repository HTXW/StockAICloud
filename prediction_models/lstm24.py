...
################################################################################
# 1) Changed: Use explicit start_date/end_date instead of period='2y'
################################################################################
ticker = 'HDFCBANK.NS'
start_date = datetime(2022, 8, 15)  # Mid-August 2022
end_date = datetime(2024, 8, 15)    # Mid-August 2024
...
################################################################################
# 2) Changed: In prepare_data, split by date (mid-April 2024) rather than 80/20
################################################################################
def prepare_data(stock_data, column):
    data = stock_data[['Date', column]]
    split_date = datetime(2024, 4, 15)  # Mid-April 2024

    # Train: mid-Aug 2022 to mid-Apr 2024
    train_data = data[data['Date'] < split_date].copy()
    # Test: mid-Apr 2024 to mid-Aug 2024
...
def plot_predictions(dates, y_true, predictions, title, save_path=None):
    plt.figure(figsize=(11, 5), dpi=450)
    ...
    plt.ylabel('Price - Indian Rupees (INR)', labelpad=8.5, fontsize=13)
    plt.legend()
    plt.grid(True)
